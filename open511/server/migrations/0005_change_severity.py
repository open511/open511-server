...
OLD_SEVERITY = dict((v, k) for k,v in NEW_SEVERITY.items())

class Migration(DataMigration):

    def forwards(self, orm):
        for rdev in orm.RoadEvent.objects.all():
            rdev.xml_data = re.sub(r'(?<=\<severity\>)\d', lambda m: NEW_SEVERITY[m.group(0)], rdev.xml_data)
            rdev.save()

    def backwards(self, orm):
        for rdev in orm.RoadEvent.objects.all():
            rdev.xml_data = re.sub(r'(?<=\<severity\>)\d', lambda m: OLD_SEVERITY[m.group(0)], rdev.xml_data)
            rdev.save()
    ...
    models = {
        ...
        u'open511.area': {
            ...
            'created': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 7, 31, 0, 0)'}),
            ...
            'updated': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 7, 31, 0, 0)'}),
